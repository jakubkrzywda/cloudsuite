...
case $2 in
  host)
    ssh $user@$master_node 'docker run -d --net host --name master cloudsuite/mahout master'
    for (( i=1; i<=$1; i+=1 )); do
    ...
    ;;
  *)
    ssh $user@$master_node 'docker run -d --net hadoop-net --name master --hostname master cloudsuite/mahout master'
    for (( i=1; i<=$1; i+=1 )); do
